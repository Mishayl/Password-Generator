...
function generatePassword() {
  ...
  if (lowerCase == true) {userChoiceArray.push(lowerCase)}
    // If upperCase == true
      // Then add upperCase array to userChoiceArray
  if (upperCase == true) {userChoiceArray.push(upperCase)}
    // If specialChar == true
      // Then add specialChar array to userChoiceArray
  if (specialChar == true) {userChoiceArray.push(specialChar)}
  ...
  for (let index = 0; index < userChoiceArray.length; index++) {
    const element = userChoiceArray[index];
    
  }
  passwordArray.push(Math.floor(Math.random() * userChoiceArray.length))
